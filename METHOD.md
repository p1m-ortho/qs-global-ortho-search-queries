...
Из 13 ни одной соответствующей семантике запросу я не увидел (см. `pubmed_result_13_medline.csv`).

Обращает внимание, что все они, будучи нерелевантными, содержат в заголовке или аннотации маску `traum*`, что заставляет усомниться в ее актуальности.

Проверяем: дополняем до этих 13 маской `traum*[tiab]` — нуль результатов.

Теперь насчет 23: из них 10 не входят в состав 13 (добавляю в коммит).

При этом из них только две ([15173946](https://pubmed.gov/15173946) и [345933](https://pubmed.gov/345933)) являются `traum*[tiab] not in[sh]`.

А теперь проверим, позволит ли добавление только `in[sh]` без `traum*[tiab]` выявить все те 4 из 45, не выдававшихся по 1419, но выдававшихся по 1464,— да, позволит, поскольку все они индексированы по этому сабмешу.

Итак, перепишем тогда снова запрос, на этот раз уже без `traum*[tiab]`:
...
(
    (
        ...
        AND
        (
            fractur*[tiab] OR injur*[tiab] OR ("Fractures, Bone"[mh:noexp] 1600/01/01:2019/04/02[mhda]) OR dislocat*[tiab] OR in[sh]
        )
        AND
        (
            thoracic[tiab] OR lumbar[tiab] OR ("Thoracic Vertebrae"[MeSH Terms] 1600/01/01:2019/04/02[mhda]) OR ("Lumbar Vertebrae"[MeSH Terms] 1600/01/01:2019/04/02[mhda]) OR thoraco*[tiab] OR t1[tiab] OR t2[tiab] OR t3[tiab] OR t4[tiab] OR t5[tiab] OR t6[tiab] OR t7[tiab] OR t8[tiab] OR t9[tiab] OR t10[tiab] OR t11[tiab] OR t12[tiab] OR th1[tiab] OR th2[tiab] OR th3[tiab] OR th4[tiab] OR th5[tiab] OR th6[tiab] OR th7[tiab] OR th8[tiab] OR th9[tiab] OR th10[tiab] OR th11[tiab] OR th12[tiab] OR d1[tiab] OR d2[tiab] OR d3[tiab] OR d4[tiab] OR d5[tiab] OR d6[tiab] OR d7[tiab] OR d8[tiab] OR d9[tiab] OR d10[tiab] OR d11[tiab] OR d12[tiab] OR l1[tiab] OR l2[tiab] OR l3[tiab] OR l4[tiab] OR l5[tiab] OR l6[tiab]
...
(
    (
        ...
        NOT
        (
            (Animals[mh:noexp] not Humans[mh:noexp]) 1600/01/01:2019/06/27[mhda]
        )
    )
    AND
    (
